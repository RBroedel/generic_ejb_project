...
import br.com.itix.comissoescnu.business.ProdutoBusiness;
import br.com.itix.comissoescnu.dto.LoadLegacyMessage;
import br.com.itix.comissoescnu.dto.ProdutoDTO;
...
public class ProdutoFacade {
	...
	public void load() {
		try {
			...
			var count = produtoBusiness.markIntegration( tag );
			...
			if( count > 0 ) {
				...
				var loadLegacyMessage = new LoadLegacyMessage<ProdutoDTO>();
				loadLegacyMessage.setTag( tag );

				for( int page = 1; page <= pages; page++ ) {

					List<Produto> legacy = produtoBusiness.getFromLegacy( tag, page, pageSize );

					loadLegacyMessage.setMessage( String.format( "Sucesso. Recebendo mensagem %d de %d.", page, pages ) );
					var listDTO = ModelMapperUtils.mapAll( legacy, ProdutoDTO.class );
					loadLegacyMessage.setLegacy( listDTO );

					try {

						new DefaultEnqueue().postMessageToQueue( queueNames.QUEUE_LOAD_PRODUTO, loadLegacyMessage, loadLegacyMessage.getClass() );

					} catch ( Exception ex ) {

						produtoBusiness.unmarkIntegration( legacy );
